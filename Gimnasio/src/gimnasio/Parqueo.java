...
public class Parqueo {
    ...
    // Método agregado para permitir asignar parqueos
    public boolean asignarEspacio(String nivel, char filaChar, int columna) {
        char[][] seleccionado = switch (nivel.toUpperCase()) {
            case "G1" -> g1;
            case "G2" -> g2;
            case "G3" -> g3;
            default -> null;
        ...
        if (fila < 0 || fila >= seleccionado.length || col < 0 || col >= seleccionado[0].length) {
            System.out.println("Fila o columna fuera de rango.");
            return false;
        }

        if (seleccionado[fila][col] == 'L') {
            seleccionado[fila][col] = 'O'; // Marcar como Ocupado
            return true;
        } else {
            System.out.println("El espacio ya está ocupado o reservado.");
            return false;
        }
    }
}
